...
|---|---|---|---|---|---:|
| P1 | 变更字段到 Survey/extra_vars 映射设计 | 定义 Survey/launch vars 结构 | 明确脚本必填参数与默认值 | 参数映射清单 v1 | 1.0 |
| P2 | Inventory 与节点角色建模 | 设计 RAC 节点组、串行策略 | 确认 node2->node1 滚动顺序要求 | Inventory 规范 | 0.8 |
| P3 | 执行账号与提权路径验证 | 验证 job 运行用户与 become_user 链路 | 验证脚本在 root/grid/oracle 切换场景 | 账号切换验证报告 | 0.8 |
| P4 | 命令封装规范定义 | 统一 shell task 模板、超时、重试 | 固化脚本返回码与关键日志关键字 | 执行封装规范 | 1.0 |
| P5 | 前置校验规则定义 | precheck gate 节点规则 | 校验脚本依赖目录、patch 包、版本配置 | 前置校验清单 | 1.0 |
...
| M5 | 日志与证据归档 | Job artifact、执行摘要模板 | 提取脚本关键输出（版本/patch结果） | 审计包模板 | 1.2 |
| M6 | 变更平台触发联调 | Webhook/API 调用 Workflow Launch | 参数一致性与幂等触发检查 | 接口联调记录 | 1.8 |
...
| L2 | 运行手册与交接 | 形成操作手册/故障SOP | 形成脚本参数速查/回滚建议 | 交付文档包 | 1.2 |
| L3 | 指标验收 | 统计执行成功率/失败分支命中 | 核验 patch 结果与版本一致性 | KPI 验收记录 | 0.8 |
...
**合计：21.1 PD（与 12 章 21.5 PD 同量级，差异来自任务颗粒度与四舍五入）。**
